...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        source=ROOT / 'data/tcp',  # file/dir/URL/glob, 0 for webcam
        ...
        project=ROOT / 'runs/output',  # save results to project/name
    ...
    # Directories
    if os.path.exists(project): # if output folder path exists / (out) 
        shutil.rmtree(project)  # delete output folder
    os.makedirs(project)  # make new output folder
    ...
    with open('runs/output/results.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    while (True):
        ...
        client,address = s.accept()                
        with client:
            transmit(client,'runs/output')
        
        if os.path.exists('data/tcp'): # if tcp folder path exists 
            shutil.rmtree('data/tcp')  # delete tcp folder
        os.makedirs('data/tcp')  # make new tcp folder
